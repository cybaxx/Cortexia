...
- **`backend/`** — FastAPI orchestrator for the independent-agent simulation path. Run: `cd backend && source .venv/bin/activate && pip install -r requirements.txt && uvicorn app.main:app --reload --port 8000`. Env: **`backend/.env`** — set **`TRIBE_MODAL_URL`** to the Modal **`extract-batch-bsv`** HTTPS URL from `modal deploy modal_app.py`; optional K2 credentials are read from `IFM_API_KEY` / `K2_THINK_API_KEY`.
